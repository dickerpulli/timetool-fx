...
public class JiraServiceImpl implements JiraService {
	...
	public WorklogInput addWorklog(String name, DateTime startDate, int minutesSpent, String comment, JiraSettings settings) {
		try {
			...
			throw new JiraBusinessException(JiraBusinessException.URL_MALFORMED, settings.getUrl());
		} catch (RestClientException e) {
			throw new JiraBusinessException(JiraBusinessException.CONNCETION_FAILURE, e, settings.getUrl());
		}
	}
		...
		if (jiraSession == null) {
			...
			try {
				...
				try {
					...
					throw new JiraBusinessException(JiraBusinessException.CONNCETION_FAILURE, e);
				} finally {
	...
	private static final class Timing {
		 ...
		 * Constructor.
		 * 
		 * @param operationDesc The description
		 */
		 ...
		 * 
		 * @param operationDesc A description
	...
	private static class JiraSession {
		 ...
		 * Constructor.
		 * 
		 * @param webServicePort The URL to connect
		 */
		public JiraSession() {
		}
		 ...
		 * @throws RemoteException if something went wrong on server
